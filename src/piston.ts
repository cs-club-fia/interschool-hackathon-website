...
// endpoint can always return clean JSON to the browser -- and so runner.ts can
// tell "the runner is down" (fall back) apart from "the student's code is
// broken" (do not fall back).
//
// The live URL is read from D1 (config.piston_url) on every run, so pointing the
// Worker at a new cloudflared hostname is a single D1 write -- NO redeploy.
...
// to Main.java when there is no public class (the default template). Note this
// is why Piston needs no `public`-stripping hack: unlike Wandbox, we choose the
// filename, so the standard `public class` pattern compiles as written.
...
// Carried over from the Wandbox runner so student-visible behaviour is
// unchanged by a backend swap: make cin throw once input is exhausted instead
// of silently spinning on a failed extraction. Prepending shifts g++ error line
// numbers by the helper's height -- that is already true today, so keeping it
// preserves parity rather than introducing a new discrepancy.
...
// Resolved Piston endpoint, or null when nothing is configured. Exposed so the
// dispatcher can skip Piston entirely (and not burn a round-trip) when it was
// never set up.
export async function pistonConfiguredUrl(env: Env): Promise<string | null> {
  let configuredUrl: string | null = null;
  try {
    configuredUrl = await getConfig(env, "piston_url");
  } catch {
    configuredUrl = null; // e.g. config table missing -- fall back below
  }
  return configuredUrl || env.PISTON_URL || null;
}
...
export async function runCode(env: Env, input: RunInput): Promise<RunResult> {
  ...
  const pistonUrl = await pistonConfiguredUrl(env);
  if (!pistonUrl) {
  ...
  const code = input.language === "cpp" ? injectCppCinExceptions(input.code) : input.code;
